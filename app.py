...
# -----------------------------------------------------------
def main():
    st.set_page_config(layout="wide", page_title="My Quant Asset Sim (V43.2)")
    ...
    st.title("💰 내 전용 퀀트 금융자산 시뮬레이터 (V43.2)")
    ...
    with c3:
        with st.container(border=True):
            ...
            use_advanced_portfolio = st.checkbox("✅ 다중 계좌 포트폴리오 (고급)", value=False, help="체크 시 아래 넓은 화면에서 계좌별로 상세 입력할 수 있습니다.")
    ...
    # 🚀 [UI 업그레이드] 다중 계좌 설정 표를 좁은 3단에서 빼내어 '가로 전체(Full-width)'로 넓게 배치
    if use_advanced_portfolio:
        with st.container(border=True):
            ...
            # use_container_width=True 로 가로 전체를 시원하게 사용
