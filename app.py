...
class FinancialSimulator:
    ...
    def run_monte_carlo(self, n_simulations=5000):
        ...
        target_legacy = self.params.get('target_legacy', 0) * 10000
        ...
        for i in range(n_simulations):
            ...
            for t, age in enumerate(years):
                ...
                base_need = (base_monthly_expense * 12) * discount_factor * lifecycle_factor + extra_expense

                if personality == '소비 집중형':
                    nominal_actual_spending = base_need + withdrawal_limit
                elif personality == '중도형':
                    nominal_actual_spending = base_need + withdrawal_limit
                else:
                    nominal_actual_spending = base_need

                if use_flex_spending and nominal_asset < initial_asset_val * 0.7:
                    nominal_actual_spending = min(nominal_actual_spending, base_need)
...
def main():
    st.set_page_config(layout="wide", page_title="Financial Asset Sim V28.1")

    st.title("💰 고정밀 금융자산 시뮬레이터 V28.1 ")
    ...
    if st.button("🚀 5,000회 시뮬레이션 시작", type="primary", use_container_width=True):
        ...
        asset_at_target = median_assets[idx_target]
        asset_at_target_worst = bottom_10_assets[idx_target]
        ...
        with g_col:
            ...
            k2.metric(f"{target_display_age}세 예상 자산 (중위값)", f"{asset_at_target:.2f}억 원")
            k3.metric(f"최악의 경우 ({target_display_age}세)", f"{asset_at_target_worst:.2f}억 원", delta_color="inverse")
            ...
            # [V28.1 NEW] 평균의 함정을 피하기 위한 로직 변경
            median_spending = np.median(sim_spending, axis=0) / 10000
            if current_age < target_display_age < death_age:
                # 1. 은퇴 전 소비액: 전체 평균 대신 "중앙값(Median)" 사용
                spend_phase1 = int(np.median(median_spending[:idx_target]))

                # 2. 은퇴 후 소비액: 말년 스파이크를 피하기 위해 "은퇴 직후 5년"의 중앙값만 사용
                idx_target_plus_5 = min(idx_target + 5, len(median_spending))
                spend_phase2_early = int(np.median(median_spending[idx_target:idx_target_plus_5]))

                st.success(f"🎯 **행동 지침 결론:** {personality}의 경우, 파산 위험을 피하려면 **현재부터 {target_display_age}세까지는 매월 약 {spend_phase1:,}만 원** 안팎으로 소비하고, **은퇴 직후 5년간({target_display_age}~{target_display_age+4}세)은 매월 약 {spend_phase2_early:,}만 원** 수준으로 관리하는 것이 가장 현실적이고 안전합니다.")
            else:
                spend_all = int(np.median(median_spending))
                st.success(f"🎯 **행동 지침 결론:** {personality}의 경우, 파산 위험을 피하려면 평생 동안 **매월 약 {spend_all:,}만 원** 안팎으로 지출을 관리하는 것이 안전합니다.")
        ...
        with d_col:
            st.subheader("📝 적용된 핵심 로직")
            st.success("""
            **1. 유동적 소비/저축 알고리즘**
            * 수입이 많을 땐 잉여 현금을 자산에 편입(저축)하고, 늘어난 자산만큼 소비 한도를 동적으로 상향시킵니다.

            **2. 수익률 체감 (Safety First)**
            * 금융자산 10억 원 초과 시 기대수익률을 점진적으로 낮춰 계산합니다.

            **3. 미래 지출 예약 (Reservation)**
            * 주택 구입 등 미래의 큰 지출을 현재 자산에서 미리 보수적으로 떼어놓고 계획을 세웁니다.
            """)
            ...
            if use_glide_path: st.info("📉 **글라이드 패스(TDF) 작동 중** (자산 노화 반영)")
            if use_flex_spending: st.success("🧠 **생존 본능(긴축) 발동 중**")
            ...
            **📊 자산 범위 ({target_display_age}세 기준)**
            * **상위 10%:** {top_10_assets[idx_target]:.2f}억 원
            * **중위 50%:** {median_assets[idx_target]:.2f}억 원
            * **하위 10%:** {bottom_10_assets[idx_target]:.2f}억 원
            """)
