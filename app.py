        ...
        for i in range(n_simulations):
            ...
            for t, age in enumerate(years):
                ...
                if personality == '소비 집중형':
                    ...
                    if remaining_years > 0 and nominal_asset > 0:
                        max_allowance = (nominal_asset * safe_rate) / (1 - (1 + safe_rate) ** -remaining_years)
                        if max_allowance > total_nominal_need:
                            nominal_actual_spending = max_allowance
                
                elif personality == '중도형':
                    if nominal_asset > 0:
                        safe_allowance = nominal_asset * 0.04
                        if safe_allowance > total_nominal_need:
                            nominal_actual_spending = safe_allowance
                ...
                cash_flow = current_nominal_income - nominal_actual_spending + nominal_lump_sum
                
                nominal_asset = nominal_asset + gain + cash_flow
